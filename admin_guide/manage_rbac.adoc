...
[[creating-derivative-cluster-role]]
== Creating a new cluster role

To create a cluster role, run the following command:

----
$ oc create clusterrole -h ...
----
...
Usage:
  oc create clusterrole NAME --verb=verb --resource=resource.group [--resource-name=resourcename] [--dry-run] [flags]
...
[[creating-derivative-cluster-role]]
== Creating a cluster role from another cluster role

. Export a cluster role to modify:
+
----
oc get clusterrole admin -o yaml > newrole.yaml
----
+
You can export any cluster role, including a
xref:../architecture/additional_concepts/authorization.adoc#roles[default cluster role],
to start your new role.

. Customize the role definition in the *_newrole.yaml_* file.
+
----
aggregationRule: <1>
...
metadata: 
  annotations: 
    openshift.io/description: "A user that has rights." <2>
    openshift.io/reconcile-protect: "false"
  creationTimestamp: 2018-04-06T15:04:05Z
  labels:
	  kubernetes.io/bootstrapping: rbac-defaults
  name: <new_cluster_role> <3>
  resourceVersion: "2321"
  selfLink: /apis/authorization.openshift.io/v1/clusterroles/<new_cluster_role> <3>
  uid: bfbf991e-39ab-11e8-bb7b-080027a54c8e
rules: <4>
- apiGroups:
  - ""
...
- apiGroups:
  ...
  - watch
----
<1> Remove the `aggregationRule` section.
<2> Optional: Update the role description.
<3> Provide the name for the new cluster role.
<4> Customize the cluster role rules as required.

. Create the role:
+
----
$ oc create -f newrole.yaml
----

////
. Grant the permissions in the cluster role to a user:
.. To grant a user permissions in all projects:
+
----
$ oc adm policy add-cluster-role-to-user <new_cluster_role> <username>
----

.. To grant a user permissions in a single project:
+
----
$ oc adm policy add-role-to-user <new_cluster_role> <username> -n <project>
----
////
