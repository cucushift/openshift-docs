...
During a cluster upgrade, the default roles
xref:../architecture/additional_concepts/authorization.adoc#roles[default
cluster roles] are automatically updated. 

[WARNING]
====
If you customized default cluster roles and want to ensure a role reconciliation
does not modify those customized roles, set the
`rbac.authorization.kubernetes.io/autoupdate` parameter to `false`.
...
[NOTE]
====
The contents of the file vary based on the OpenShift version, but the file 
contains only the default policies.
====

. Update the *_policy.json_* file to include any cluster role customizations.

. Use the policy file to automatically reconcile the roles and role bindings:
+
----
